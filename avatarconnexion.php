                ...
                if($width_orig >= 500 && $height_orig >= 500 && $width_orig <= 6000 && $height_orig <= 6000){ // On vérifie que la taille de l'image et correcte
                    ...
                    if ($_FILES['file']['size'] <= $tailleMax){ // Si le fichier et bien de taille inférieur ou égal à 5 Mo
                        ...
                        if (in_array($extensionUpload, $extensionsValides)){ // Vérifie que l'extension est correct
                            ...
                            $dossier = "public/avatars/" . $_SESSION['id_etudiant'] . "/"; // On se place dans le dossier de la personne 
 
                            if (!is_dir($dossier)){ // Si le nom de dossier n'existe pas alors on le crée
                                mkdir($dossier);
                            }else{
                                if(file_exists("public/avatars/". $_SESSION['id_etudiant'] . "/" . $_SESSION['avatar']) && isset($_SESSION['avatar'])){
                                    unlink("public/avatars/". $_SESSION['id_etudiant'] . "/" . $_SESSION['avatar']);
                                }
                            }
 
                            $nom = md5(uniqid(rand(), true)); // Permet de générer un nom unique à la photo
                            $chemin = "public/avatars/" . $_SESSION['id_etudiant'] . "/" . $nom . "." . $extensionUpload; // Chemin pour placer la photo
                            ...
                            if ($resultat){ // Si on a le résultat alors on va comprésser l'image
                                ...
                                if (is_readable("public/avatars/" . $_SESSION['id_etudiant'] . "/" .$nom . "." . $extensionUpload)) {
                                    $verif_ext = getimagesize("public/avatars/" . $_SESSION['id_etudiant'] . "/" .$nom . "." . $extensionUpload);
                                    ...
                                    if($verif_ext['mime'] == $ListeExtension[$extensionUpload]  || $verif_ext['mime'] == $ListeExtensionIE[$extensionUpload]){              
                                        ...
                                        $filename = "public/avatars/" . $_SESSION['id_etudiant'] . "/" .$nom . "." . $extensionUpload;
                                        ...
                                        if($extensionUpload == 'jpg' || $extensionUpload == 'jpeg' || $extensionUpload == "pjpg" || $extensionUpload == 'pjpeg'){
                                            ...
                                            // Affichage
                                            imagejpeg($image_p2, "public/avatars/" . $_SESSION['id_etudiant'] . "/" . $nom . "." . $extensionUpload, 75);
                                            imagedestroy($image_p2);
                                        }


                                        $updateavatar = $connect->prepare('UPDATE etudiant SET avatar = ? WHERE id_etudiant = ?');
            $updateavatar->execute(array(($nom.".".$extensionUpload), $_SESSION['id_etudiant']));
            // header('Location: profil.php?id_etudiant='.$_SESSION['id_etudiant']);

 
                                        // $connect->query("UPDATE etudiant SET avatar = ? WHERE  id_etudiant = ?", 
                                            // array(($nom.".".$extensionUpload), $_SESSION['id_etudiant']));
